...
The script now targets the fast accelerated path by default: `FA3` auto when available, `FP8` on for pretraining, `device-batch-size=16`, auto total batch size, and activation checkpointing off. Quality still depends on the training recipe, total tokens, data mixture, and evaluation results.
