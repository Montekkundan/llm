...
export PICOLLM_DEVICE_BATCH_SIZE=8
export PICOLLM_TRAIN_LOSS_CHUNK_ROWS=8
...
- `PICOLLM_DEVICE_BATCH_SIZE=8` is the current default for the fast H100 path.
- `PICOLLM_TRAIN_LOSS_CHUNK_ROWS=8` reduces the LM-head loss projection peak memory.
...
The script now targets the fast accelerated path by default: `FA3` auto when available, `FP8` on for pretraining, `device-batch-size=8`, auto total batch size, loss chunks of `8`, and activation checkpointing off. Quality still depends on the training recipe, total tokens, data mixture, and evaluation results.
...
- `python -m picollm.accelerated.pretrain.train --depth=24 --target-param-data-ratio=8 --device-batch-size=8 --fp8 --wandb-entity "$WANDB_ENTITY"`
- `python -m picollm.accelerated.pretrain.eval --device-batch-size=8`
- `python -m picollm.accelerated.chat.sft --device-batch-size=8 --wandb-entity "$WANDB_ENTITY"`
