...
def read_system_prompt(args) -> str:
...
def append_user_turn(tokens: list[int], text: str, tokenizer, user_start: int, user_end: int) -> None:
    tokens.append(user_start)
    tokens.extend(tokenizer.encode(text))
    tokens.append(user_end)


def generation_budget_error(prompt_tokens: list[int], max_tokens: int, context_window: int) -> str | None:
    ...
    if max_tokens > remaining_tokens:
        return (
            f"Requested --max-tokens={max_tokens}, but only {remaining_tokens} tokens remain in the "
...
def main(
    ...
    *,
    ...
    model, tokenizer, meta = load_model_fn(args.source, device, phase="eval", model_tag=args.model_tag, step=args.step)
    ...
    engine = engine_cls(model, tokenizer)
    ...
    while True:
        ...
        append_user_turn(candidate_tokens, user_input, tokenizer, user_start, user_end)
        candidate_tokens.append(assistant_start)
        budget_error = generation_budget_error(candidate_tokens, args.max_tokens, engine.model.config.sequence_len)
